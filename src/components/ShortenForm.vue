<template>
  <form @submit.prevent="onSubmit">
    ...
    <div v-if="!noExpire" class="mb-4">
      ...
      <input v-model="expiresAt" id="expiresAt" type="datetime-local" name="expiresAt"
        class="w-full p-3 border rounded" />
    </div>
    <button type="submit"
      class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg p-3 shadow transition">Shorten</button>
    <div v-if="result" class="mt-4 text-center">
      <span v-if="result.shortUrl" class="font-semibold">Shortened URL:</span>
      <a v-if="result.shortUrl" :href="result.shortUrl" class="text-blue-600 underline" target="_blank">{{
        result.shortUrl }}</a>
